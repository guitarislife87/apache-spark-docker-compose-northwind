...
psql -v ON_ERROR_STOP=1 --username "${POSTGRES_USER}" -d "${SPARK_DB}" <<-EOSQL
   GRANT ALL ON SCHEMA public TO ${SPARK_USER};
   CREATE SCHEMA northwind;
   GRANT ALL ON SCHEMA northwind TO ${SPARK_USER};
EOSQL
...
psql -v ON_ERROR_STOP=1 --username "${POSTGRES_USER}" -d "${SPARK_DB}" <<-EOSQL
    ...
    SET search_path TO northwind_dw;

    -- Table: northwind_dw.date_dimension

    -- DROP TABLE IF EXISTS northwind_dw.date_dimension;

    CREATE TABLE IF NOT EXISTS northwind_dw.date_dimension
    (
        ...
        CONSTRAINT date_dimension_id_pk PRIMARY KEY (date_dimension_id)
    )

    TABLESPACE pg_default;

    -- Index: date_dimension_date_actual_idx

    -- DROP INDEX IF EXISTS northwind_dw.date_dimension_date_actual_idx;

    CREATE INDEX IF NOT EXISTS date_dimension_date_actual_idx
        ON northwind_dw.date_dimension USING btree
    ...
    -- DROP TABLE IF EXISTS northwind_dw.order_facts;

    CREATE TABLE IF NOT EXISTS northwind_dw.order_facts
    (
        order_id integer NOT NULL,
        customer_id text COLLATE pg_catalog."default",
        employee_id smallint,
        order_date integer references date_dimension(date_dimension_id),
        required_date integer references date_dimension(date_dimension_id),
        shipped_date integer references date_dimension(date_dimension_id),
        ship_via smallint,
        freight real,
        ship_name text COLLATE pg_catalog."default",
        ship_address text COLLATE pg_catalog."default",
        ship_city text COLLATE pg_catalog."default",
        ship_region text COLLATE pg_catalog."default",
